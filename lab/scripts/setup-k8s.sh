...
# AWS creds for Bedrock (auto-injected by the aws_accounts resource).
mkdir -p /root/.aws
cat > /root/.aws/credentials << CREDEOF
[default]
aws_access_key_id = ${INSTRUQT_AWS_ACCOUNT_BEDROCK_AWS_ACCESS_KEY_ID:-}
aws_secret_access_key = ${INSTRUQT_AWS_ACCOUNT_BEDROCK_AWS_SECRET_ACCESS_KEY:-}
CREDEOF
printf '[default]\nregion = us-east-1\n' > /root/.aws/config
